...
import sys, os, sqlite3, pickle, utils, numpy as np, pandas as pd
...
# Load fully aligned features/MS2 spectra ########################################################################
full = pickle.load(open("fullFeatures_htan_IROA_c18_target.pickle", "rb"))
paramFile = paramFile = "/Research/Projects/7Metabolomics/Dev/JUMPm_unlabel_python/htan/c18/jumpm_positive.params"
libFile = "library.db"
##################################################################################################################
...
for i in range(nFeatures):
    ...
    for j in range(nCompounds):
        ...
        if mzDiff < matchMzTol:
            ...
            print(i, j, fMeanMz[i], libMz[j], fNewRt[i], libRt[j], sim)
