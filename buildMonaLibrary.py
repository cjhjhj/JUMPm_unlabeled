            ...
            if mass != "NA":
                ...
                if dictMs2 is not None:
                    ...
                    for i in range(len(dictMs2["mz"])):
                        listMs2.append({"id": uid, "mz": str(dictMs2["mz"][i]), "intensity": str(dictMs2["intensity"][i])})
                    conn.executemany('''INSERT INTO ms2 (id, mz, intensity) VALUES (:id, :mz, :intensity);''', listMs2)
                    conn.commit()
