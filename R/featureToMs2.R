...
args = commandArgs(T)
paramFile = "C:/Research/Projects/7Metabolomics/JUMPm/IROAsamples/jumpm_negative_desktop.params"
featureFile = "C:/Research/Projects/7Metabolomics/Dev/JUMPm_unlabel_python/IROA_IS_NEG/.IROA_IS_NEG_fully_aligned.feature"
mzXMLs = "C:/Research/Projects/7Metabolomics/JUMPm/IROAsamples/IROA_IS_NEG_1.mzXML,C:/Research/Projects/7Metabolomics/JUMPm/IROAsamples/IROA_IS_NEG_2.mzXML,C:/Research/Projects/7Metabolomics/JUMPm/IROAsamples/IROA_IS_NEG_3.mzXML"
mzXMLs = unlist(strsplit(mzXMLs, ","))
outDirectory = "C:/Research/Projects/7Metabolomics/Dev/JUMPm_unlabel_python/IROA_IS_NEG/MS2"
logFile = "C:/Research/Projects/7Metabolomics/Dev/JUMPm_unlabel_python/IROA_IS_NEG/jumpm.log"
LOG = file(logFile, "w")
...
for (m in 1:length(mzXMLs)) {
    ...
    # Compatibility with old-formatted .feature file(s)
    for (i in 1:ncol(df)) {
    ...
    ##############################################################
    pb = txtProgressBar(1, nrow(h), width = 20, style = 3)
    ...
    for (i in 1:nrow(h)) {
        ...
        if (h$msLevel[i] == 1) {
            ...
            ind = which(i > df$`minMS1` & i < df$`maxMS1` &
                            df$PercentageTF < pctTfThreshold &
    ...
    ################################################
    pb = txtProgressBar(1, dim(featureToScanNum)[1], width = 20, style = 3)
...
cat("    Merging MS2 spectra between runs for each feature\n", file = LOG)
pb = txtProgressBar(1, length(featureToSpectra), width = 20, style = 3)
